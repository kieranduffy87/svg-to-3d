...
// ─── Thin-stroke icons ──────────────────────────────────────────────────────
const Icon = ({ d, d2, viewBox = '0 0 24 24', ...props }) => (
  <svg width="18" height="18" viewBox={viewBox} fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d={d} />{d2 && <path d={d2} />}
  </svg>
)

const Icons = {
  Geometry: () => <Icon d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" />,
  Material: () => (
  ...
  Background: () => <Icon d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z" d2="M4 22v-7" />,
  Animation: () => <Icon d="M5 3l14 9-14 9V3z" />,
  Export: () => <Icon d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" d2="M7 10l5 5 5-5M12 15V3" />,
}
...
// ─── Section Panels ──────────────────────────────────────────────────────────
function GeometryPanel({ settings, updateSetting }) {
  return (
...
function MaterialPanel({ settings, updateSetting, setSettings }) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: 20 }}>
        {Object.entries(MATERIAL_PRESETS).map(([key, { label }]) => (
          <button key={key} onClick={() => applyPreset(key)} title={label} style={{
            ...
            onMouseEnter={e => { if (settings.materialPreset !== key) e.currentTarget.style.background = 'rgba(255,255,255,0.05)' }}
            onMouseLeave={e => { if (settings.materialPreset !== key) e.currentTarget.style.background = 'transparent' }}
          >
            <div className={MATERIAL_PREVIEWS[key]} style={{ width: 26, height: 26, borderRadius: '50%' }} />
...
function BackgroundPanel({ settings, updateSetting }) {
  return (
    <div>
...
function AnimationPanel({ settings, updateSetting }) {
  return (
    <div>
...
function ExportPanel({ settings, updateSetting, hasMesh, exporting, setExporting }) {
    ...
    <div>
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 14 }}>
        <button disabled={!hasMesh} onClick={() => window.dispatchEvent(new CustomEvent('export-glb'))} style={{
          ...
          onMouseEnter={e => { if (hasMesh) e.currentTarget.style.color = 'rgba(255,255,255,0.8)' }}
...
const NAV_ITEMS = [
  ...
  { key: 'background', label: 'Background', Ico: Icons.Background },
        ...
        {NAV_ITEMS.map(({ key, label, Ico }) => {
          ...
          return (
            ...
            >
              ...
              {label}
              {active && (
                <div style={{ marginLeft: 'auto', width: 4, height: 4, borderRadius: '50%', background: 'rgba(255,255,255,0.4)' }} />
              )}
            </button>
          )
        })}
